-- Do not manually edit this file, it was auto-generated by dillonkearns/elm-graphql
-- https://github.com/dillonkearns/elm-graphql


module Hasura.Object.Message_max_fields exposing (..)

import Graphql.Internal.Builder.Argument as Argument exposing (Argument)
import Graphql.Internal.Builder.Object as Object
import Graphql.Internal.Encode as Encode exposing (Value)
import Graphql.Operation exposing (RootMutation, RootQuery, RootSubscription)
import Graphql.OptionalArgument exposing (OptionalArgument(..))
import Graphql.SelectionSet exposing (SelectionSet)
import Hasura.InputObject
import Hasura.Interface
import Hasura.Object
import Hasura.Scalar
import Hasura.Union
import Json.Decode as Decode
import ScalarCodecs


created_at : SelectionSet (Maybe ScalarCodecs.Timestamptz) Hasura.Object.Message_max_fields
created_at =
    Object.selectionForField "(Maybe ScalarCodecs.Timestamptz)" "created_at" [] (ScalarCodecs.codecs |> Hasura.Scalar.unwrapCodecs |> .codecTimestamptz |> .decoder |> Decode.nullable)


id : SelectionSet (Maybe ScalarCodecs.Uuid) Hasura.Object.Message_max_fields
id =
    Object.selectionForField "(Maybe ScalarCodecs.Uuid)" "id" [] (ScalarCodecs.codecs |> Hasura.Scalar.unwrapCodecs |> .codecUuid |> .decoder |> Decode.nullable)


media_id : SelectionSet (Maybe ScalarCodecs.Uuid) Hasura.Object.Message_max_fields
media_id =
    Object.selectionForField "(Maybe ScalarCodecs.Uuid)" "media_id" [] (ScalarCodecs.codecs |> Hasura.Scalar.unwrapCodecs |> .codecUuid |> .decoder |> Decode.nullable)


priority : SelectionSet (Maybe Int) Hasura.Object.Message_max_fields
priority =
    Object.selectionForField "(Maybe Int)" "priority" [] (Decode.int |> Decode.nullable)


tag_id : SelectionSet (Maybe ScalarCodecs.Uuid) Hasura.Object.Message_max_fields
tag_id =
    Object.selectionForField "(Maybe ScalarCodecs.Uuid)" "tag_id" [] (ScalarCodecs.codecs |> Hasura.Scalar.unwrapCodecs |> .codecUuid |> .decoder |> Decode.nullable)


text : SelectionSet (Maybe String) Hasura.Object.Message_max_fields
text =
    Object.selectionForField "(Maybe String)" "text" [] (Decode.string |> Decode.nullable)


updated_at : SelectionSet (Maybe ScalarCodecs.Timestamptz) Hasura.Object.Message_max_fields
updated_at =
    Object.selectionForField "(Maybe ScalarCodecs.Timestamptz)" "updated_at" [] (ScalarCodecs.codecs |> Hasura.Scalar.unwrapCodecs |> .codecTimestamptz |> .decoder |> Decode.nullable)


user_id : SelectionSet (Maybe String) Hasura.Object.Message_max_fields
user_id =
    Object.selectionForField "(Maybe String)" "user_id" [] (Decode.string |> Decode.nullable)
